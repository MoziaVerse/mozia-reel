...
async def _provider(db_session, models):
...
class TestSyncGatewayModels:
    async def test_adds_newly_listed_models(self, db_session):
        p, repo = await _provider(db_session, [_row("old/a")])
        await sync_gateway_models(db_session, provider_id=p.id, rows=[_row("old/a"), _row("new/b")])
        assert {m.model_id for m in await repo.list_models(p.id)} == {"old/a", "new/b"}

    async def test_delisted_model_is_disabled_not_deleted(self, db_session):
        """项目里可能还引用着它，删掉会让那些项目的模型字段指向空气。"""
        p, repo = await _provider(db_session, [_row("gone/x"), _row("stay/y")])
        await sync_gateway_models(db_session, provider_id=p.id, rows=[_row("stay/y")])
    ...
    async def test_endpoint_follows_the_platform(self, db_session):
        """endpoint 决定请求打到哪条路径，错了就是必然失败——不是用户偏好能覆盖的。"""
        p, repo = await _provider(db_session, [_row("voxcpm2", endpoint="openai-chat")])
        await sync_gateway_models(db_session, provider_id=p.id, rows=[_row("voxcpm2", endpoint="openai-tts")])
        assert (await repo.list_models(p.id))[0].endpoint == "openai-tts"

    async def test_user_default_choice_survives_sync(self, db_session):
        """哪个当默认是用户设的，对账不该把它抹掉。"""
        p, repo = await _provider(db_session, [_row("z/chat", is_default=True)])
        await sync_gateway_models(db_session, provider_id=p.id, rows=[_row("z/chat")])
        assert (await repo.list_models(p.id))[0].is_default is True

    async def test_relisted_model_is_re_enabled(self, db_session):
        p, repo = await _provider(db_session, [_row("back/x", is_enabled=False)])
        await sync_gateway_models(db_session, provider_id=p.id, rows=[_row("back/x")])
        assert (await repo.list_models(p.id))[0].is_enabled is True

    async def test_durations_follow_the_endpoint_change(self, db_session):
        """从视频改判成非视频时，时长预设要一并清掉，否则留着一份对不上的数据。"""
        p, repo = await _provider(db_session, [_row("x/y", endpoint="openai-video", supported_durations="[4, 8]")])
    ...
    async def test_empty_catalog_is_a_no_op(self, db_session):
        ...
        p, repo = await _provider(db_session, [_row("keep/x")])
        await sync_gateway_models(db_session, provider_id=p.id, rows=[])
        assert (await repo.list_models(p.id))[0].is_enabled is True
