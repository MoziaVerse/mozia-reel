...
def _applied_revisions(db_path: Path) -> set[str]:
    """alembic_version 表里当前的版本号（分支存在时不止一行）。"""
...
def test_full_chain_downgrade_only_drops_intentional_indexes(alembic_cfg):
    ...
    for revision in (s.revision for s in script.walk_revisions("base", "heads")):
        # 本发行版的链上有一个合并节点（把上游链与 matrix 链并起来），于是 walk 顺序会在
        # 两条分支间交错。降到合并节点的某个父版本时，另一条分支会被一并撤销 —— 那些版本
        # 在后续轮次里已不是当前版本，再对它们 downgrade 会因"目标不是祖先"报错。
        # 所以按 alembic_version 的实际状态推进，撤销过的直接跳过。
        if revision not in _applied_revisions(db_path):
            continue
        down_revision = script.get_revision(revision).down_revision or "base"
        # 合并节点的 down_revision 是父版本元组；降到其中一个父版本即可，alembic 会把
        # 合并节点连同另一条分支一起撤销。
        target = down_revision[0] if isinstance(down_revision, tuple) else str(down_revision)
