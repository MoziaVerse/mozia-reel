...
@pytest.fixture
def svc(db_session: AsyncSession) -> ConfigService:
...
class TestVoiceOmittedFromPayload:
    async def test_empty_voice_is_not_sent(self):
        ...
        backend._client = type(
            "C", (), {"audio": type("A", (), {"speech": type("S", (), {"create": staticmethod(fake_create)})()})()}
        )()
        from lib.narration_delivery import MODEL_DEFAULT_VOICE

        req = AudioSynthesisRequest(text="你好", output_path=Path("/tmp/x.mp3"), voice=MODEL_DEFAULT_VOICE)
