...
class TestWalletTokenStorage:
    ...
    async def test_roundtrip(self, session):
        await save_wallet_token(session, "wt-abc")
        assert await get_wallet_token(session) == "wt-abc"

    @pytest.mark.asyncio
    async def test_cleared_when_matrix_omits_it(self, session):
        """matrix 不再下发时要清掉旧值，否则会拿着过期 token 一直查失败。"""
        await save_wallet_token(session, "wt-abc")
        await save_wallet_token(session, None)
        assert await get_wallet_token(session) is None
