...
class TestSyncGatewayModels:
    ...
    async def test_quota_sources_follow_the_platform(self, session):
        """网关随时可以调整某型号能吃哪个分区；存量行不跟着更新，标签就停在握手那一刻。"""
        p, repo = await _provider(session, [_row("x/y", quota_sources=["gift", "paid"])])
        await sync_gateway_models(session, provider_id=p.id, rows=[_row("x/y", quota_sources=["paid"])])
        assert (await repo.list_models(p.id))[0].quota_sources == ["paid"]

    async def test_rows_without_the_key_clear_the_partition(self, session):
        """回落路径（网关 /v1/models 发现）的行本就没有分区信息，不该留着上一次的标注。"""
        p, repo = await _provider(session, [_row("x/y", quota_sources=["gift"])])
        await sync_gateway_models(session, provider_id=p.id, rows=[_row("x/y")])
