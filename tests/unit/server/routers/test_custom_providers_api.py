...
class TestQuotaSourcesSurviveModelSave:
    ...
    async def _seed_quota(self, db_session: AsyncSession, pid: int, model_id: str, sources: list[str]):
    ...
    async def test_replace_models_keeps_quota_sources(
        ...
        await self._seed_quota(db_session, pid, "gpt-4o", ["gift", "paid"])
    ...
    async def test_full_update_keeps_quota_sources(self, custom_providers_client: TestClient, db_session: AsyncSession):
        pid = custom_providers_client.post("/api/v1/custom-providers", json=_PROVIDER_PAYLOAD).json()["id"]
        await self._seed_quota(db_session, pid, "gpt-4o", ["paid"])
