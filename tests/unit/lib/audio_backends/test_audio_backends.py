...
class TestOpenAIAudioBackend:
    ...
    def test_list_voices_keeps_official_catalog_for_openai_itself(self):
        ...
        上游同位置的 ``..._returns_full_catalog_for_custom_openai_tts_endpoint`` 与
        ``..._legacy_narrowing_only_applies_to_official_openai`` 断言的是相反口径
        （自定义 endpoint 保持全量目录）。本发行版的网关自建 TTS 对任何 preset voice
        都返回 400，那两条在此不成立，故不采纳。
        """
        with captured_openai_clients():
            from lib.audio_backends.openai import OpenAIAudioBackend
