...
class TestSynthesisUsesReferenceAudio:
    async def _synth(self, tmp_path: Path, voice: str) -> dict:
        ...
        with patch("lib.openai_shared.AsyncOpenAI"):
            ...
            with patch.object(backend, "_post_speech_raw", new=AsyncMock(return_value=b"audio")) as raw:
                await backend.synthesize(AudioSynthesisRequest(text="喂", output_path=tmp_path / "o.wav", voice=voice))
    ...
    async def test_unknown_voice_id_still_sent_as_preset(self, library, tmp_path):
        ...
        with patch("lib.openai_shared.AsyncOpenAI"):
            ...
            out = tmp_path / "o.wav"
            await backend.synthesize(AudioSynthesisRequest(text="hi", output_path=out, voice="alloy"))

            # 产出：未知 id 没有被当成库音色拦下，合成照常落盘。
            assert out.read_bytes() == b"audio"
            # 形态：原样作为 preset voice 发出，且不夹带克隆用的 ref_audio。
