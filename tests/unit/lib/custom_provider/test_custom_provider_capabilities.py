...
class TestSystemCapabilities:
    def test_openai_video_reads_caps_per_model_from_the_backend(self):
        """openai-video 按 model 读 backend 声明，而不是 endpoint 维度的硬上限。

        本发行版这条 endpoint 上同时挂着真 Sora（首帧单槽，上限 1）与经网关过来的
        MiniMax H3（上限 9），endpoint 维度写死一个数会让其中一方谎报能力。
        """
