...
async def fetch_window(
    token: str, *, start: int, end: int, total_budget_seconds: float = _TOTAL_BUDGET_SECONDS
) -> GatewayWindow:
    ...
    ``total_budget_seconds`` 是显式注入缝：默认取模块常量，用例传一个很小的值来验证
    超预算时带 truncated 收手，不必去改模块状态。
    """
    if start > end:
        start, end = end, start
    ...
    async with httpx.AsyncClient(timeout=_REQUEST_TIMEOUT_SECONDS) as client:
        for page in range(1, _MAX_PAGES + 1):
            if time.monotonic() > deadline:
                truncated = True
                logger.warning("平台流水取数超出总预算 %.0fs，对账结果不完整", total_budget_seconds)
                break
            try:
